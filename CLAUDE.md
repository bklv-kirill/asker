...
- **Per-user дебаунс ассистент-турнов (`internal/telegram/debounce.go`):** оба входа в LLM-флоу (`handler_assistant.go` для текста, `handler_voice.go` после успешной транскрибации) пушат сообщение в общий per-user воркер через `t.submitToDebounce(b, from, chatID, usersID, text)` вместо прямого вызова `processAssistantTurn`. Воркер копит `[]string` буфер, и каждый новый push сбрасывает окно `debounceWindow = 1500ms`; по истечении окна склеивает буфер через `\n\n` и зовёт `processAssistantTurn` — это даёт сериализацию параллельных LLM-вызовов одного юзера и склейку «лесенки» из быстрых сообщений в один логический turn. **Окно фиксированное 1500ms**, без динамики (в моменте дополнительная сложность не нужна — typing-индикатор стартует сразу при первом push'е и закрывает UX-восприятие задержки). **Idle-таймаут `debounceIdleTimeout = 5min`** — после паузы такой длины воркер самоликвидируется и удаляется из map'ы под `debounceMu`; на следующее сообщение пересоздаётся. **Drop-семантика**: при любом command/callback/pending хендлере (start, setup_profile, my_profile, attach_phone, contact, profile_set_age/gender/info, profile_gender, pending_input) рядом с `clearPendingInput(from.ID)` ставится `dropUserDebounce(from.ID)` — он сигнализирует воркеру очистить буфер (а не флашить его), потому что callback/command — это явное переключение контекста, накопленный текст уже неактуален. **Typing внутри воркера** (а не в `processAssistantTurn`): запускается на первом push'е в пустой буфер, останавливается после возврата из turn'а; в `processAssistantTurn` typing намеренно не дёргается — иначе будет дубль. **Graceful shutdown — два контекста**: `Start` сохраняет signal-ctx в `t.rootCtx` (для select-кейсов «пора выходить» в воркерах) и СОЗДАЁТ независимый `t.processCtx` через `context.WithCancel(context.Background())` — он передаётся во ВСЕ in-flight операции turn'а (`llm.Prompt`, `b.SendMessage`, typing, `chatMessages.Create`). На SIGINT отменяется только rootCtx — воркеры на `<-shutdownCtx.Done()` флашат остаток буфера через `processAssistantTurn(processCtx, ...)` и выходят; in-flight LLM/SendMessage не отменяются и доводят ответ до юзера. После `b.Start(ctx)` зовётся `shutdownDebounce(debounceShutdownTimeout = 360s)` — ждёт `WaitGroup` или таймаут, в обоих случаях в конце зовёт `processCancel()` (cleanup при штатном done, force-cancel при таймауте). Если добавляется новый in-flight ресурс (новый LLM-провайдер, S3, и т.п.) — он принимает processCtx, не rootCtx. Контракты слоёв при склейке: `chat_messages` пишется ДВУМЯ записями (role=user со склеенным текстом + role=assistant с ответом) ТОЛЬКО ПОСЛЕ успешного LLM — инвариант «либо пара, либо ничего» (на сбое LLM в БД не остаётся осиротевших вопросов; raw журнал в `telegram_events.message_in` остаётся N штук — по одной на физическое сообщение от TG); `message_out` — один на ответ ассистента. Дебаунс — деталь Telegram-слоя, не самостоятельный домен; если позже понадобится дебаунс в HTTP-слое или иной точке — отрефакторим в отдельный пакет.
- **Graceful shutdown инфраструктура (air + compose):** PID 1 в dev-контейнере — `air` (CMD в Dockerfile). По умолчанию air шлёт SIGKILL в Go-бинарник без ожидания, и весь Go-уровень graceful-кода никогда не выполняется. В `.air.toml` настроены `send_interrupt = true` (SIGINT вместо SIGKILL — main.signal.NotifyContext поймает) и `kill_delay = "365s"` (>= debounceShutdownTimeout). В `docker-compose.yaml` — `init: true` (tini как PID 1, корректное forwarding'е сигналов) и `stop_grace_period: 375s` (>= air.kill_delay). Цепочка таймаутов: compose 375s ≥ air 365s ≥ Go 360s; при изменении одного — синхронить остальные. **ВАЖНО:** при foreground `docker compose up` повторный Ctrl+C = force-kill (SIGKILL), graceful не сработает — для гарантированного graceful использовать `docker compose stop` из другого терминала. Дополнительно: `bot.WithHTTPClient(5*time.Second, ...)` поджимает long-poll timeout с дефолтных 60s до 5s — иначе idle-shutdown через прокси висит до минуты на отмене HTTP-запроса. **ОГРАНИЧЕНИЕ:** air's `runner/util_linux.go` (v1.65.1) при killCmd walk'ит `/proc/<pid>/task/<pid>/children` рекурсивно и шлёт SIGINT каждому потомку индивидуально по PID — `SysProcAttr.Setpgid: true` на subprocess (`internal/services/ai/claude_code_cli/`) защищает от kill-by-pgroup, но НЕ от air. Setpgid оставлен как hardening против других signal-источников; при добавлении новых форкающих провайдеров — ставить такой же. Для production-deploy (Phase 4) air заменится прямым запуском бинарника + tini, и проблема уйдёт.
...
- **STT (распознавание голосовых, Фаза 2):** параллельный пакет `internal/services/stt` (файл `stt.go`): тип `Audio{Bytes, MimeType, Filename}`, `Info{Provider, Model}`, интерфейс `STT` (`GetInfo()` идёт первым, затем `Transcribe(ctx, Audio) (string, error)`). Реализации провайдеров — в подпапках `internal/services/stt/<name>/` (сейчас один — `groq/` поверх Groq Cloud, free-tier, OpenAI-совместимый `/openai/v1/audio/transcriptions`, модель `whisper-large-v3-turbo`). Сборка по `STT_PROVIDER` — в фабрике `internal/services/stt/factory/factory.go` (симметрично `internal/services/ai/factory`): константы `ProviderGroq` и т.п., `NewSTT(provider, apiKey, model, timeout) stt.STT` со switch и panic при неизвестном provider. Добавление второго провайдера (OpenAI Whisper, self-hosted whisper.cpp) — правка фабрики + новый подпакет, без правок main.go. Голосовой хендлер `internal/telegram/handler_voice.go` отдельным файлом — matchMessageVoice по `Message.Voice` (audio/video/video_note намеренно не ловим), скачивание voice через `b.GetFile` + `b.FileDownloadLink`, MIME hardcoded "audio/ogg" + Filename "voice.ogg" (без расширения Groq возвращает 400 «could not determine file format»). Лимит длительности (`STT_MAX_DURATION_SEC`) проверяется ДО STT, гейт привязки номера — ДО скачивания, чтобы не жечь квоту. Расшифровка идёт в существующий ассистент-флоу через общий метод `processAssistantTurn` (отрефакторен из `handleAssistant` 2026-04-27); `chat_messages.role = user` хранит расшифрованный текст без маркировки источника. На любой ошибке STT/скачивания/пустой расшифровке — fallback «не получилось распознать», `voice_in` в журнал НЕ пишется. STT-конфиг (`STT_*`) — все поля безусловно обязательные.
...
2. **Никогда не делать коммиты без прямой команды пользователя.** Не коммитить «по ходу задачи», не коммитить «на всякий случай», не коммитить даже если задача кажется завершённой и зелёной. Коммит — **только** по явному запросу пользователя («закоммить», «commit», «закоммить изменения по инструкции» и т.п.). До этого момента изменения остаются в working tree, а пользователь сам решает, когда фиксировать. Это переопределяет общие best-practices про «коммить по ходу» — в этом проекте они не применяются.
3. **Когда пользователь явно попросил коммит — делать логическими единицами.** Файлы, связанные по логике и смыслу, идут в один коммит; несвязанные изменения — разными коммитами. Не сваливать всё в один «big bang» и не плодить мусорные микрокоммиты. Один коммит = одна смысловая единица.
...
5. **Описание коммита** — краткое, **на русском языке**, только суть изменения. Без даты, автора, `Co-Authored-By`, ссылок на задачи, эмодзи и прочего служебного шума. Только что сделано.
