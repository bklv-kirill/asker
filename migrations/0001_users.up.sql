...
CREATE TABLE IF NOT EXISTS users (
    ...
    info        TEXT,
    -- Номер телефона. Обязательное поле: запись users появляется ТОЛЬКО после
    -- того, как пользователь привязал номер (TG-юзер до этого момента живёт
    -- в telegram_users без связи с users). UNIQUE — один номер не может быть
    -- закреплён за двумя записями. CHECK гарантирует, что в колонке только
    -- цифры (без +, скобок, пробелов, дефисов): нормализация — на стороне
    -- приложения, БД отбраковывает мусор как страховку. Минимальная длина
    -- проверяется только как «не пусто».
    phone       TEXT     NOT NULL UNIQUE
                         CHECK (length(phone) > 0 AND phone NOT GLOB '*[^0-9]*'),
